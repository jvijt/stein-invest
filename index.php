...
function spark(points,purchases=[],avgCost=null){
 ...
 if(Number.isFinite(Number(avgCost))&&Number(avgCost)>0){
   min=Math.min(min,Number(avgCost));
   max=Math.max(max,Number(avgCost));
 }
 ...
 let extra='';
 if(Number.isFinite(Number(avgCost))&&Number(avgCost)>0){
   const y=h-(Number(avgCost)-min)/span*h;
   extra+=`<line x1="0" y1="${y.toFixed(1)}" x2="${w}" y2="${y.toFixed(1)}" stroke="#f5b942" stroke-width="1" stroke-dasharray="4 4" opacity=".72" vector-effect="non-scaling-stroke"><title>Gemiddelde aankoopprijs ${euro(Number(avgCost))}</title></line>`;
 }
...
function renderSectorPie(positions){
  const m={}; let total=0;
  positions.forEach(x=>{
    const v=Number(x.quote?.price_eur||0)*Number(x.qty||0);
    const sec=x.sector||'Overig'; m[sec]=(m[sec]||0)+v; total+=v;
  });
  const rows=Object.entries(m).filter(([,v])=>v>0).sort((a,b)=>b[1]-a[1]);
  const pie=document.getElementById('sectorPie'), legend=document.getElementById('sectorLegend');
  if(!rows.length||!total){pie.innerHTML='<div class="small">Nog geen sectorgegevens.</div>';legend.innerHTML='';return;}
  let a=-Math.PI/2, paths='';
  rows.forEach(([name,val],i)=>{
    const frac=val/total, b=a+frac*Math.PI*2;
    const x1=100+82*Math.cos(a),y1=100+82*Math.sin(a),x2=100+82*Math.cos(b),y2=100+82*Math.sin(b);
    const large=frac>0.5?1:0;
    paths+=`<path d="M100 100 L${x1.toFixed(2)} ${y1.toFixed(2)} A82 82 0 ${large} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z" fill="${sectorColors[i%sectorColors.length]}"><title>${esc(name)} ${(frac*100).toFixed(1)}%</title></path>`;
    a=b;
  });
  paths+=`<circle cx="100" cy="100" r="47" fill="#0d1824"/><text x="100" y="96" text-anchor="middle" fill="#91a7ba" font-size="10">Sectoren</text><text x="100" y="114" text-anchor="middle" fill="#f5f8fb" font-size="16" font-weight="800">${rows.length}</text>`;
  pie.innerHTML=`<svg viewBox="0 0 200 200" role="img" aria-label="Taartgrafiek sectorverdeling">${paths}</svg>`;
  legend.innerHTML=rows.map(([name,val],i)=>`<div class="legend-row"><i class="swatch" style="background:${sectorColors[i%sectorColors.length]}"></i><span>${esc(name)}</span><b>${(val/total*100).toFixed(1)}%</b></div>`).join('');
}

async function loadDashboard(){
 const p=document.getElementById('platform').value;
 const r=await fetch('api.php?action=dashboard&platform='+encodeURIComponent(p),{credentials:'same-origin'});
 const d=await r.json(); if(!d.ok){alert(d.error||'Fout');return} csrf=d.csrf||csrf;
 lastDashboardData=d; lastPositions=d.positions||[]; renderFromCache();
...
function renderFromCache(){
 ...
 ps.forEach(x=>{
   x._cost=Number(x.cost_eur);x._pnl=x._value-x._cost;x._pnlPct=x._cost?x._pnl/x._cost*100:0;
   x._weight=provisionalTotal?x._value/provisionalTotal*100:0;x._dayPct=Number(x.quote?.day_pct||0);
 });
 const mode=document.getElementById('sortOrder')?.value||'weight_desc';
 const cmp={
   weight_desc:(a,b)=>b._weight-a._weight, weight_asc:(a,b)=>a._weight-b._weight,
   pnl_pct_desc:(a,b)=>b._pnlPct-a._pnlPct, pnl_pct_asc:(a,b)=>a._pnlPct-b._pnlPct,
   pnl_eur_desc:(a,b)=>b._pnl-a._pnl, pnl_eur_asc:(a,b)=>a._pnl-b._pnl,
   value_desc:(a,b)=>b._value-a._value, value_asc:(a,b)=>a._value-b._value,
   day_desc:(a,b)=>b._dayPct-a._dayPct, day_asc:(a,b)=>a._dayPct-b._dayPct,
   name_asc:(a,b)=>(a.name||'').localeCompare(b.name||''), name_desc:(a,b)=>(b.name||'').localeCompare(a.name||'')
 }[mode];
 if(cmp)ps.sort(cmp);
 lastPositions=ps; renderSectorPie(ps); if(document.body.classList.contains('tv'))applyTVLayout();

 let total=0,cost=0,day=0,prevTotal=0,largest=null; const mix={}; const cards=document.getElementById('cards');cards.innerHTML='';
 ps.forEach(x=>{
   ...
   const olderPurchases=purchases.filter(b=>b.date && b.date<todayStr);
   const hasOlderPosition=olderPurchases.length>0;
   ...
   const dayPos=(hasOlderPosition && price>0 && prevPrice>0) ? (price-prevPrice)*eligibleQty : 0;
   const prevValue=(hasOlderPosition && prevPrice>0) ? prevPrice*eligibleQty : 0;
   total+=value;cost+=c;day+=dayPos;prevTotal+=prevValue;
   if(!largest||value>largest.value)largest={name:x.name,value};
   const plats=x.platforms?x.platforms.join(' + '):x.platform;
   if(x.platform_qty && Object.keys(x.platform_qty).length){
     Object.entries(x.platform_qty).forEach(([z,qy])=>{mix[z]=(mix[z]||0)+price*Number(qy||0);});
   }else{
     const z=x.platform||((x.platforms||[])[0]||'');
     if(z)mix[z]=(mix[z]||0)+value;
   }
   const open=!!q.is_open, marketText=open?'Beurs open':'Beurs gesloten';
   cards.insertAdjacentHTML('beforeend',`<article class="card"><div class="top"><div><div class="ticker">${esc(x.ticker)}</div><div class="name">${esc(x.name)}</div></div><div class="badge">${esc(plats)}</div></div>
   <div class="price">${price?euro(price):'Geen koers'} <span class="${cls(Number(q.day_pct||0))}" style="font-size:12px">${pct(Number(q.day_pct||0))}</span>
   <span class="market"><i class="dot ${open?'open':''}"></i>${marketText}</span></div>
   <div class="stats"><div class="stat"><span>Aantal</span><b>${Number(x.qty).toLocaleString('nl-BE',{maximumFractionDigits:6})}</b></div>
   <div class="stat"><span>Gem. aankoop (EUR)</span><b>${euro(Number(x.avg_cost_eur))}</b></div>
   <div class="stat"><span>Waarde</span><b>${euro(value)}</b></div>
   <div class="stat"><span>Winst/verlies</span><b class="${cls(pl)}">${euro(pl)} · ${pct(plp)}</b></div>
   <div class="stat"><span>Gewicht portefeuille</span><b>${x._weight.toFixed(1)}%</b></div>
   </div>
   <div class="spark">${spark(q.points,x.purchases||[],Number(x.avg_cost_eur||0))}</div><div class="small">${esc(q.exchange||x.exchange_name||'')} · ${esc(q.currency||x.currency||'')}</div></article>`);
 });
 const pl=total-cost,plp=cost?pl/cost*100:0,dp=prevTotal?day/prevTotal*100:0;
 document.getElementById('total').textContent=euro(total);document.getElementById('invested').textContent='Geïnvesteerd: '+euro(cost);document.getElementById('pnl').textContent=euro(pl);document.getElementById('pnl').className='big '+cls(pl);document.getElementById('pnlpct').textContent=pct(plp);
 document.getElementById('day').textContent=euro(day);document.getElementById('day').className='big '+cls(day);document.getElementById('daypct').textContent=pct(dp)+' sinds vorige slotkoers';
 document.getElementById('largest').textContent=largest?largest.name:'—';document.getElementById('largestpct').textContent=largest&&total?(largest.value/total*100).toFixed(1)+'% van portefeuille':'—';
 document.getElementById('mix').textContent=Object.entries(mix).map(([k,v])=>k+' '+euro(v)).join(' · ')||'—';
}
function applyColumnCount(){
 const n=parseInt(document.getElementById('columnCount')?.value||'3',10);
...
async function loadSectors(){
  try{
    ...
    if(!d.ok)return;
    ['sector','epSector'].forEach(id=>{
      const el=document.getElementById(id); if(!el)return;
      const current=el.value;
      el.innerHTML=d.sectors.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join('');
      if(d.sectors.includes(current))el.value=current; else if(d.sectors.includes('Overig'))el.value='Overig';
    });
    return d.sectors;
  }catch(e){}
}
async function openSectors(){
  ...
  sectorManageList.innerHTML=sectors.map(x=>`<span style="display:inline-block;border:1px solid var(--line);border-radius:999px;padding:5px 9px;margin:3px">${esc(x)}</span>`).join('');
  sectorDialog.showModal();
}
async function addSector(){
  const name=newSectorName.value.trim(); if(!name)return;
  const fd=new FormData();fd.append('csrf',csrf);fd.append('name',name);
  const r=await fetch('api.php?action=add_sector',{method:'POST',body:fd,credentials:'same-origin'}),d=await r.json();
  if(!d.ok){alert(d.error||'Toevoegen mislukt');return}
  newSectorName.value='';await openSectors();
}
async function suggestSector(){
  ...
  try{
    ...
    if([...sector.options].some(o=>o.value===d.sector)) sector.value=d.sector;
    sectorSuggestion.innerHTML=`Advies: <b>${esc(d.sector)}</b> — ${esc(d.reason)} <span style="color:var(--muted)">(je kunt dit wijzigen)</span>`;
  }catch(e){}
}
...
assetSearch.addEventListener('input',()=>{
  verifiedAsset=null;ticker.value='';assetName.value='';assetVerified.classList.remove('show');
  clearTimeout(assetSearchTimer);
  const q=assetSearch.value.trim();
  if(q.length<2){assetSearchResults.innerHTML='';return;}
  assetSearchTimer=setTimeout(()=>searchAssets(q),250);
});
document.addEventListener('click',e=>{if(!e.target.closest('.searchbox'))assetSearchResults.innerHTML='';});
async function searchAssets(q){
  assetSearchResults.innerHTML='<div class="small" style="padding:10px">Zoeken…</div>';
  try{
    const r=await fetch('api.php?action=asset_search&q='+encodeURIComponent(q),{credentials:'same-origin'});const d=await r.json();
    if(!d.ok)throw new Error(d.error||'Zoeken mislukt');
    if(!d.results?.length){assetSearchResults.innerHTML='<div class="small" style="padding:10px">Geen bestaand aandeel of ETF gevonden.</div>';return;}
    assetSearchResults.innerHTML=d.results.map(x=>`<button type="button" class="search-item" data-symbol="${esc(x.symbol)}" data-name="${esc(x.name)}"><b>${esc(x.name)}</b><div class="search-meta">${esc(x.symbol)} · ${esc(x.exchange||'')} · ${esc(x.type||'')}</div></button>`).join('');
    assetSearchResults.querySelectorAll('.search-item').forEach(btn=>btn.addEventListener('click',()=>selectAsset(btn.dataset.symbol,btn.dataset.name)));
  }catch(e){assetSearchResults.innerHTML='<div class="small" style="padding:10px">Zoeken tijdelijk niet beschikbaar.</div>'}
}
async function selectAsset(symbol,name){
  assetSearchResults.innerHTML='<div class="small" style="padding:10px">Controleren…</div>';
  try{
    const r=await fetch('api.php?action=asset_validate&ticker='+encodeURIComponent(symbol),{credentials:'same-origin'});const d=await r.json();
    if(!d.ok)throw new Error(d.error||'Validatie mislukt');
    verifiedAsset=d.asset;ticker.value=d.asset.ticker;assetName.value=d.asset.name||name;assetSearch.value=(d.asset.name||name)+' ('+d.asset.ticker+')';
    if([...currency.options].some(o=>o.value===d.asset.currency))currency.value=d.asset.currency;
    assetVerifiedText.textContent=`${d.asset.ticker} · ${d.asset.exchange||''} · ${d.asset.currency||''}`;assetVerified.classList.add('show');assetSearchResults.innerHTML='';await suggestSector();
  }catch(e){verifiedAsset=null;ticker.value='';assetVerified.classList.remove('show');assetSearchResults.innerHTML='<div class="small" style="padding:10px">Dit effect kon niet worden gevalideerd.</div>'}
}
...
async function saveTx(){
 if(!verifiedAsset || !ticker.value){alert('Zoek het aandeel op naam, ISIN of ticker en selecteer eerst een geverifieerd resultaat.');return}
 const fd=new FormData();fd.append('csrf',csrf);['ticker','tradeDate','price','quantity'].forEach(()=>{});
 fd.append('ticker',ticker.value.trim());fd.append('name',assetName.value.trim());fd.append('sector',sector.value);fd.append('currency',currency.value);fd.append('trade_date',tradeDate.value);fd.append('price',price.value);fd.append('quantity',quantity.value);fd.append('platform',txPlatform.value);fd.append('type',type.value);
 const r=await fetch('api.php?action=save_transaction',{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();
 if(!d.ok){alert(d.error||'Opslaan mislukt');return} addDialog.close(); verifiedAsset=null;ticker.value=assetName.value=assetSearch.value=price.value=quantity.value='';assetVerified.classList.remove('show'); await loadDashboard();
}

async function openPositions(){
  const r=await fetch('api.php?action=positions_raw',{credentials:'same-origin'});
  const d=await r.json(); if(!d.ok){alert(d.error||'Fout');return} csrf=d.csrf||csrf;
  let h='<table><tr><th>Aandeel</th><th>Platform</th><th>Aantal</th><th>Gem. aankoop</th><th>Valuta</th><th>Sector</th><th></th></tr>';
  (d.positions||[]).sort((a,b)=>(a.name||'').localeCompare(b.name||'') || (a.platform||'').localeCompare(b.platform||'')).forEach(x=>{
    const payload={asset_id:Number(x.asset_id),name:x.name,platform:x.platform,qty:Number(x.qty),avg:Number(x.avg_cost_native??x.avg_cost_eur),currency:x.currency||'EUR',sector:x.sector||'Overig'};
    h+=`<tr><td><b>${esc(x.name)}</b><div class="small">${esc(x.ticker)}</div></td><td><b>${esc(x.platform)}</b></td><td>${Number(x.qty).toLocaleString('nl-BE',{maximumFractionDigits:6})}</td><td>${Number(x.avg_cost_native??x.avg_cost_eur).toLocaleString('nl-BE',{maximumFractionDigits:6})}</td><td>${esc(x.currency||'EUR')}</td><td>${esc(x.sector||'Overig')}</td><td><button onclick='editPosition(${JSON.stringify(payload).replace(/'/g,"&#39;")})'>Aanpassen</button></td></tr>`;
  });
  h+='</table>';posList.innerHTML=h;
  if(!posDialog.open) posDialog.showModal();
}
async function openNewPosition(){
  const r=await fetch('api.php?action=assets',{credentials:'same-origin'});const d=await r.json();
  if(!d.ok){alert(d.error||'Aandelen laden mislukt');return} csrf=d.csrf||csrf;
  epMode.value='new';epAssetId.value='';epExistingField.style.display='none';epNewField.style.display='';deletePositionBtn.style.display='none';
  epAssetSelect.innerHTML=(d.assets||[]).map(a=>`<option value="${Number(a.id)}" data-name="${esc(a.name)}" data-currency="${esc(a.currency||'EUR')}" data-sector="${esc(a.sector||'Overig')}">${esc(a.name)} (${esc(a.ticker)})</option>`).join('');
  if(!epAssetSelect.options.length){alert('Er zijn nog geen aandelen gekend. Voeg eerst een aandeel toe via + Transactie.');return}
  const opt=epAssetSelect.options[0];epCurrency.value=opt.dataset.currency||'EUR';epSector.value=opt.dataset.sector||'Overig';
  epPlatform.value='ING';epQty.value='';epAvg.value='';epDate.value=new Date().toISOString().slice(0,10);
  editPosDialog.showModal();
}
epAssetSelect.addEventListener('change',()=>{
  const opt=epAssetSelect.options[epAssetSelect.selectedIndex];
  if(opt){epCurrency.value=opt.dataset.currency||'EUR';epSector.value=opt.dataset.sector||'Overig';}
});
function editPosition(x){
  epMode.value='edit';epExistingField.style.display='';epNewField.style.display='none';deletePositionBtn.style.display='';
  epAssetId.value=x.asset_id;epName.value=x.name;epPlatform.value=x.platform;epQty.value=x.qty;epAvg.value=x.avg;epCurrency.value=x.currency||'EUR';epDate.value=new Date().toISOString().slice(0,10);epSector.value=x.sector||'Overig';
  editPosDialog.showModal();
}

async function deletePosition(){
  if(epMode.value!=='edit') return;
  const name=epName.value||'deze positie';
  const platform=epPlatform.value;
  if(!confirm(`Positie ${name} op ${platform} volledig verwijderen? Dit verwijdert alle transacties voor deze positie op dit platform.`)) return;
  const fd=new FormData();fd.append('csrf',csrf);fd.append('asset_id',epAssetId.value);fd.append('platform',platform);
  const r=await fetch('api.php?action=delete_position',{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();
  if(!d.ok){alert(d.error||'Verwijderen mislukt');return}
  editPosDialog.close();
  await loadDashboard();
  await openPositions();
}
...
async function savePosition(){
  const isNew=epMode.value==='new';
  const assetId=isNew?epAssetSelect.value:epAssetId.value;
  if(!assetId){alert('Selecteer een aandeel.');return}
  const fd=new FormData();fd.append('csrf',csrf);fd.append('asset_id',assetId);fd.append('platform',epPlatform.value);fd.append('quantity',epQty.value);fd.append('avg_cost_native',epAvg.value);fd.append('currency',epCurrency.value);fd.append('trade_date',epDate.value);
  let r=await fetch('api.php?action=set_position',{method:'POST',body:fd,credentials:'same-origin'});let d=await r.json();if(!d.ok){alert(d.error||'Opslaan mislukt');return}

  const afd=new FormData();afd.append('csrf',csrf);afd.append('id',assetId);
  if(isNew){
    const opt=epAssetSelect.options[epAssetSelect.selectedIndex];
    afd.append('name',opt?.dataset.name||opt?.textContent||'Aandeel');
  }else{
    afd.append('name',epName.value);
  }
  afd.append('sector',epSector.value);afd.append('currency',epCurrency.value);
  r=await fetch('api.php?action=save_asset',{method:'POST',body:afd,credentials:'same-origin'});d=await r.json();if(!d.ok){alert(d.error||'Sector opslaan mislukt');return}
...
async function openUsers(){
  ...
  if(!d.ok){alert(d.error||'Fout');return} csrf=d.csrf||csrf;
  let h='<table><tr><th>Gebruiker</th><th>Rol</th><th>Nieuw wachtwoord</th><th></th></tr>';
  (d.users||[]).forEach(u=>{
    h+=`<tr><td><b>${esc(u.username)}</b></td>
    <td><select id="role_${u.id}"><option value="guest" ${u.role==='guest'?'selected':''}>Guest</option><option value="admin" ${u.role==='admin'?'selected':''}>Admin</option></select></td>
    <td><input id="pw_${u.id}" type="password" placeholder="Leeg = behouden" style="min-width:160px;background:#07131e;color:#fff;border:1px solid var(--line);padding:8px;border-radius:8px"></td>
    <td><button onclick="saveUser(${Number(u.id)})">Opslaan</button> <button onclick="deleteUser(${Number(u.id)})">Verwijder</button></td></tr>`;
  });
  h+='</table>';userList.innerHTML=h;userDialog.showModal();
}
async function createUser(){
  const fd=new FormData();fd.append('csrf',csrf);fd.append('username',newUsername.value.trim());fd.append('password',newPassword.value);fd.append('role',newRole.value);
  const r=await fetch('api.php?action=create_user',{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();
  if(!d.ok){alert(d.error||'Aanmaken mislukt');return}
  newUsername.value='';newPassword.value='';newRole.value='guest';await openUsers();
}
async function saveUser(id){
  const fd=new FormData();fd.append('csrf',csrf);fd.append('id',id);fd.append('role',document.getElementById('role_'+id).value);fd.append('password',document.getElementById('pw_'+id).value);
  const r=await fetch('api.php?action=update_user',{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();
  if(!d.ok){alert(d.error||'Opslaan mislukt');return}await openUsers();
}
async function deleteUser(id){
  if(!confirm('Deze gebruiker verwijderen?'))return;
  const fd=new FormData();fd.append('csrf',csrf);fd.append('id',id);
  const r=await fetch('api.php?action=delete_user',{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();
  if(!d.ok){alert(d.error||'Verwijderen mislukt');return}await openUsers();
}

async function openTransactions(){
 const r=await fetch('api.php?action=transactions',{credentials:'same-origin'});const d=await r.json();csrf=d.csrf||csrf;
 let h='<table><tr><th>Datum</th><th>Aandeel</th><th>Type</th><th>Aantal</th><th>Koers</th><th>Platform</th><th></th></tr>';
 (d.transactions||[]).forEach(t=>h+=`<tr><td>${esc(t.trade_date)}</td><td><b>${esc(t.name)}</b><div class="small">${esc(t.ticker)}</div></td><td>${t.type==='BUY'?'Koop':'Verkoop'}</td><td>${Number(t.quantity)}</td><td>${Number(t.price_native).toLocaleString('nl-BE')} ${esc(t.currency)}</td><td>${esc(t.platform)}</td><td><button onclick="deleteTx(${Number(t.id)})">Wis</button></td></tr>`);
 h+='</table>';txList.innerHTML=h;txDialog.showModal();
}
...
function applyTVLayout(){
  ...
  document.documentElement.style.setProperty('--tv-cols',best.cols);
  document.documentElement.style.setProperty('--tv-rows',best.rows);
  document.body.classList.toggle('tv-dense', n>12);
}
...
function toggleTV(){
  ...
  if(document.body.classList.contains('tv')){
    ...
    setTimeout(()=>{applyTVLayout();renderFromCache();},180);
  }else{
    document.exitFullscreen?.();
    document.documentElement.style.removeProperty('--tv-cols');
    document.documentElement.style.removeProperty('--tv-rows');
    setTimeout(()=>renderFromCache(),100);
  }
}
